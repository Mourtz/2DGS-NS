...
class ClusteredMaterialField(nn.Module):
    def __init__(self, n_clusters: int = 12, roughness_init=None, metallic_init=None, freeze_materials: bool = False):
        super().__init__()
    ...
    @torch.no_grad()
    def cluster(self, xyz: torch.Tensor, albedo: torch.Tensor = None, pos_weight: float = 1.0, color_weight: float = 3.0):
        import numpy as np
        ...
        if albedo is not None:
            ...
            features = np.concatenate([xyz_np / pos_std * pos_weight, albedo.detach().float().cpu().numpy() * color_weight], axis=1)
        else:
            features = xyz_np

        km = KMeans(n_clusters=self.n_clusters, n_init=10, random_state=42)
        labels = km.fit_predict(features)
        centers = np.stack([xyz_np[labels == k].mean(axis=0) if (labels == k).any() else xyz_np.mean(axis=0) for k in range(self.n_clusters)])
        ...
        logger.info("[ClusteredMaterialField] K=%d clusters (%s%s) | active=%d/%d | roughness [%.2f,%.2f] metallic [%.2f,%.2f]",
                    self.n_clusters, 'xyz+albedo' if albedo is not None else 'xyz only', alb_info, n_used, self.n_clusters, r.min(), r.max(), m.min(), m.max())
...
class ClusteredCookTorranceShader(nn.Module):
    def __init__(self, n_lights: int = 2, n_clusters: int = 12, roughness_init=None, metallic_init=None, freeze_materials: bool = False):
        super().__init__()
        self.cmf = ClusteredMaterialField(n_clusters=n_clusters, roughness_init=roughness_init, metallic_init=metallic_init, freeze_materials=freeze_materials)
...
class DirectionalLightModel(nn.Module):
    def __init__(self, n_lights: int = 2):
        ...
        init_dirs[0] = torch.tensor([ 0.5,  1.0,  0.3])
        if n_lights > 1:
            init_dirs[1] = torch.tensor([-0.3,  0.5, -0.2])
        self._dirs = nn.Parameter(init_dirs)
        init_log_i = torch.zeros(n_lights, 3)
        init_log_i[0] = torch.log(torch.tensor([1.1, 1.1, 1.1]))
        if n_lights > 1:
            init_log_i[1] = torch.log(torch.tensor([0.4, 0.4, 0.5]))
...
class CookTorranceShader(nn.Module):
    ...
    @staticmethod
    def _smith_ggx(NdotV: torch.Tensor, NdotL: torch.Tensor, roughness: torch.Tensor) -> torch.Tensor:
        k   = (roughness + 1.0) ** 2 / 8.0
        g_v = NdotV / (NdotV * (1.0 - k) + k + 1e-7)
        g_l = NdotL / (NdotL * (1.0 - k) + k + 1e-7)
        return g_v * g_l

    def forward(self, xyz: torch.Tensor, albedo: torch.Tensor, normal: torch.Tensor, view_dir: torch.Tensor) -> torch.Tensor:
...
class SphMipEnvironment(nn.Module):
    ...
    def query(self, refl_dir: torch.Tensor, roughness: torch.Tensor) -> torch.Tensor:
        ...
        sampled = F.grid_sample(self.envmap, grid, mode='bilinear', padding_mode='border', align_corners=True)
        return F.softplus(sampled.squeeze(0).squeeze(-1).squeeze(-1).T)
...
class CookTorranceProbeShader(nn.Module):
    def __init__(self, nmf_hidden: int = 64, nmf_pos_freqs: int = 6, probe_resolution: int = 64, probe_update_interval: int = 500, probe_n_blur: int = 5):
        super().__init__()
        from scene.neural_brdf_field import NeuralMaterialField
        self.nmf = NeuralMaterialField(hidden=nmf_hidden, n_pos_freqs=nmf_pos_freqs)
        from scene.env_probe import EnvironmentProbe
        self.probe = EnvironmentProbe(resolution=probe_resolution, update_interval=probe_update_interval, n_blur_levels=probe_n_blur)
...
class CookTorranceRefGSShader(nn.Module):
    def __init__(self, nmf_hidden: int = 64, nmf_pos_freqs: int = 6, feat_dim: int = 8, n_mip_levels: int = 8, env_h: int = 8, env_w: int = 16):
        super().__init__()
        from scene.neural_brdf_field import NeuralMaterialField
        self.nmf = NeuralMaterialField(hidden=nmf_hidden, n_pos_freqs=nmf_pos_freqs)
        ...
        self.mlp = nn.Sequential(nn.Linear(in_dim, hidden), nn.ReLU(inplace=True), nn.Linear(hidden, 3))
...
class CookTorranceSphMipShader(nn.Module):
    def __init__(self, nmf_hidden: int = 64, nmf_pos_freqs: int = 6, n_mip_levels: int = 8, env_h: int = 64, env_w: int = 128):
        super().__init__()
        from scene.neural_brdf_field import NeuralMaterialField
        self.nmf = NeuralMaterialField(hidden=nmf_hidden, n_pos_freqs=nmf_pos_freqs)
